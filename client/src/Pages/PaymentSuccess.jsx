...
function PaymentSuccess() {
  ...
  // console.log(fullData);
  return (
    <div class="container mt-4 mb-4 lg:w-4/5 mx-auto px-2 py-4 bg-white rounded-lg shadow-md">
      <div class="flex items-center justify-center flex-col mb-4">
        <h1 class="text-2xl font-bold text-gray-800">Booking Confirmation</h1>
        <img
          ...
          alt="Logo"
          class="w-40 h-32 mt-2"
        />
      </div>

      <div class="mb-8 lg:w-3/4 mx-auto">
        <h2 class="text-lg font-semibold mb-2">Ticket Details</h2>
        <div class="bg-gray-100 p-4 rounded-lg grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="font-semibold">Ticket ID:</div>
          <div>{ticketId ? ticketId : ""}</div>
          <div class="font-semibold">Payment ID:</div>
          <div>{transactionId}</div>
          <div class="font-semibold">Bus Name:</div>
          <div>{fullData.data?.name}</div>
          <div class="font-semibold">Passenger Details:</div>
          <div>
          ...
          <div class="font-semibold">Seat Booked:</div>
      ...
      <div class="w-full lg:w-3/4 mx-auto">
        <h2 class="text-lg font-semibold mb-2">Bus Details</h2>
        <div class="bg-gray-100 p-4 rounded-lg shadow-md">
          <h3 class="text-xl font-semibold">{fullData.data?.name}</h3>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:flex lg:flex-row items-center justify-between mb-4">
            <div class="mb-2 lg:mb-0">
              <p class="text-gray-600">Rating: {fullData.data?.rating}</p>
            </div>
            <div>
          ...
          <p class="text-gray-600 mb-4">{fullData.data?.category}</p>
          <div class="grid grid-cols-1 md:grid-cols-2 lg:flex lg:flex-row justify-between mb-4">
            <div class="mb-2 lg:mb-0">
              <p class="text-gray-600">{fullData.data?.from}</p>
              <p class="text-gray-600">( Mahavir Bus Station )</p>
            </div>
            <div>
              <p class="text-gray-600">{fullData.data?.to}</p>
              <p class="text-gray-600">( South Civil Line )</p>
            </div>
          </div>
          <div class="flex items-center flex-wrap">
            <p class="text-gray-600 mr-2">Amenities:</p>
            <ul class="flex space-x-2">
              {fullData.data?.amenities?.map((amenity, index) => (
                <li key={index} class="text-blue-500">
                  {amenity}
                </li>
              ))}
            </ul>
